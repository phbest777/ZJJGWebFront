<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="支付到账日期:" prop="hxtransdate">
        <el-input
          v-model.trim="form.hxtransdate"
          autocomplete="off"
          :disabled="!inputEditable"
        ></el-input>
      </el-form-item>
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="submitZfCheckRedo">确 定</el-button>
    </div>
  </el-dialog>
</template>
...
<script>
  ...
  import { CheckZfInfoRedo } from '@/api/xzhouse_zjzf'
  ...
  export default {
    ...
    methods: {
      ...
      submitZfCheckRedo() {
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            if (this.title === '资金支付确认发送') {
              this.$confirm('你确定要执行该操作', '资金支付确认发送', {
                ...
                .then(async () => {
                  ...
                  const { msg } = await CheckZfInfoRedo(this.form)
                  this.$baseMessage(msg, 'success')
                  this.$refs['form'].resetFields()
                  this.dialogFormVisible = false
